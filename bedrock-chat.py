...
def query_llm(params, handler):
    ...
    if "claude" in params['model']:
        model='us.anthropic.'+params['model']
    else:
        model=params['model']+"-instruct-v1:0"
        

    if "sonnet" in model or "haiku" in model:
        model+="-20240620-v1:0" if "claude-3-5-sonnet" in model else "-20241022-v1:0" if "claude-3-5-haiku" in model else  "-20240307-v1:0" if "haiku" in model else "-20240229-v1:0"
        claude3=True
