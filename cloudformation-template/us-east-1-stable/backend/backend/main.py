...
# Agent ARNs
CODE_GEN_AGENT_ARN = 'arn:aws:bedrock-agentcore:us-east-1:025523569182:runtime/code_generation_agent-xyz'  # Shared
...
async def get_settings():
    """Get current settings - returns actual runtime config with env vars applied"""
    config = load_config()
    
    # Override with environment variables (what's actually being used at runtime)
    if os.getenv("RAY_CODE_GEN_AGENT_ARN"):
        if "global" not in config:
            config["global"] = {}
        config["global"]["code_gen_agent_arn"] = os.getenv("RAY_CODE_GEN_AGENT_ARN")
    
    if os.getenv("RAY_SUPERVISOR_ARN"):
        if "ray" not in config:
            config["ray"] = {}
        config["ray"]["supervisor_arn"] = os.getenv("RAY_SUPERVISOR_ARN")
    
    if os.getenv("SPARK_SUPERVISOR_ARN"):
        if "spark" not in config:
            config["spark"] = {}
        config["spark"]["supervisor_arn"] = os.getenv("SPARK_SUPERVISOR_ARN")
...
async def generate_spark_code(request: SparkGenerateRequest):
    ...
    try:
        ...
        config = load_config()
        s3_bucket = config.get("spark", {}).get("s3_bucket", "spark-data-025523569182-us-east-1")
...
async def upload_spark_csv(request: CsvUploadRequest):
    ...
    try:
        s3_client = boto3.client('s3', region_name='us-east-1')
        bucket = "spark-data-025523569182-us-east-1"
