...
def bedrock_claude_(params,chat_history,system_message, prompt,model_id,image_path=None, handler=None):
    ...
    response = bedrock_runtime.converse_stream(messages=chat_history_copy, modelId=model_id,inferenceConfig={"maxTokens": 3000, "temperature": 0.5,},system=system_message)
...
def self_crtique(params,code, error,file_names,handler=None):
    import re
    prompt=f"""I will provide you a pyspark code that analyzes a tabular data and an error relating to the code. 
...
Provide the fixed code within <code> XML tags and all python top-level package names (seperated by comma, no extra formatting) needed within <package> XML tags"""
    model_id='us.anthropic.'+params['model']

    if "sonnet" in model_id or "haiku" in model_id:
        model_id+="-20240620-v1:0" if "claude-3-5-sonnet" in model_id else "-20241022-v1:0" if "claude-3-5-haiku" in model_id else  "-20240307-v1:0" if "haiku" in model_id else "-20240229-v1:0"
...
You will have access to a set of "tools" for handling specific request, use your judgement to figure out if you need to use a tool and what tool to use. I will provide the tool description below that guides you on if and when to use a tool:
    1. pyspark_function_tool: This tool is used to handle structured data files to perform any data analysis query and task on such files (CSV, XLSX, etc.). Structure data will usually be tagged by the file name and will be in a CSV string.
...
If a user query does not need a tool, go ahead an answer the question directly without using any tool. Do not include any preamble""",
    ...
    tool_description="""This tool allows you to analyze structured data files (CSV, PARQUET, XLSX, etc.) using PySpark programming language. It can be used to answer questions or perform analyses on the data contained in these files.
...
Input: Structured data file(s) (CSV, XLS, XLSX, PARQUET)
Processing:
- Read input files from Amazon S3:
    ...
    XLS/XLSX files: Load using Excel-specific methods, e.g., spark.read_excel('s3a://path/to/your/file.xlsx', sheet_name='Sheet3' )
    ...
    if stop_reason != "tool_use":
        ...
        for content in message['content']:
            if 'toolUse' in content:
                ...
                if tool['name'] == 'pyspark_function_tool':
