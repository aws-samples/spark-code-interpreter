...
5. Call execute_spark_code_emr for validation execution - THIS RETURNS IMMEDIATELY WITH job_run_id
6. STOP AND WAIT - The execute_spark_code_emr result contains job_state field
   - If job_state == 'SUCCESS': Execution completed successfully, proceed to step 7
   - If job_state == 'FAILED': Execution failed, proceed to step 8
   - DO NOT call any other tools until you check job_state
   - DO NOT regenerate code until you verify current execution failed
7. IF job_state == 'SUCCESS':
...
8. IF job_state == 'FAILED' AND attempts < 3:
   - Call extract_execution_logs to analyze error
   - Go back to step 2 with error feedback
   - Then execute new code starting from step 5
...
3. Call extract_python_code to clean the code - STORE this as your validated_code variable
4. Call validate_spark_code to check basic requirements
5. ONLY IF validation passed: Call execute_spark_code_emr for validation execution
6. WAIT for execution to complete - check result.job_state == 'SUCCESS' or 'FAILED'
7. IF execution succeeded (job_state == 'SUCCESS'):
   - Call extract_execution_logs to get logs
   - Call fetch_spark_results to get output data
   - Return final JSON response with validated_code
8. IF execution failed (job_state == 'FAILED') AND attempts < 3:
   - Call extract_execution_logs to analyze error
   - Go back to step 2 with error feedback
   - DO NOT submit new execution until previous one completes
...
- PostgreSQL tables provided: {bool(selected_postgres_tables)} → Use EMR (call execute_spark_code_emr)
- Glue tables provided: {bool(selected_tables)} → Use EMR (call execute_spark_code_emr)
- S3 CSV file provided: {bool(s3_input_path)} → Use select_execution_platform
- No data sources: Use Lambda (call execute_spark_code_lambda)
