...
DYNAMODB_TABLE=`echo $DYNAMODB_TABLE | sed "s/{{ACCOUNT_ID}}/${AWS_ACCOUNT_ID}/g" | sed "s/{{REGION}}/${AWS_REGION}/g"`
USER_ID=$(jq -r '.UserId' config.json)
BUCKET_NAME=$(jq -r '.Bucket_Name' config.json)
BUCKET_NAME=`echo $BUCKET_NAME | sed "s/{{ACCOUNT_ID}}/${AWS_ACCOUNT_ID}/g" | sed "s/{{REGION}}/${AWS_REGION}/g"`
INPUT_BUCKET=$(jq -r '.input_bucket' config.json)
INPUT_BUCKET=`echo $INPUT_BUCKET | sed "s/{{ACCOUNT_ID}}/${AWS_ACCOUNT_ID}/g" | sed "s/{{REGION}}/${AWS_REGION}/g"`
